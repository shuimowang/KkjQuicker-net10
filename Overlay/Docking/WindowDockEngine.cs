...
namespace KkjQuicker.Overlay.Docking
{
    /// <summary>
    /// 提供默认单例形式的窗口贴边管理入口。
    /// <para>
    ...
    public sealed class WindowDockEngine : IDisposable
    {
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(WindowDockEngine));
        }
        ...
        private void RestoreOne(WindowDockInfo dw, bool tryBringToFront, bool animate)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        RestoreHiddenState(hwnd, dw.OriginalShownOnTaskbar);
        ...
        private void HideToEdge(WindowDockInfo dw, IntPtr hwnd, RECT bounds, RECT monitor)
        {
            ...
            // ↓ 修复：去掉 OriginalShownOnTaskbar 限制，对所有窗口（包括 ShowInTaskbar="False" 的 WPF
            //   窗口）统一隐藏 Alt+Tab 条目；恢复时由 RestoreHiddenState 按原始状态还原，不会误加任务栏条目
    ...
    double springZeta = 0.86)
        ...
        /// <summary>
        /// 将窗口的 Alt+Tab / 任务栏扩展样式还原为加入管理前的状态。
        /// <para>
        /// 若窗口原本显示在任务栏中，则完整还原（移除 WS_EX_TOOLWINDOW，补回 WS_EX_APPWINDOW）；
        /// 若原本不在任务栏中（如设置了 ShowInTaskbar="False" 的 WPF 窗口），
        /// 则仅移除引擎添加的 WS_EX_TOOLWINDOW，不额外补 WS_EX_APPWINDOW，
        /// 避免将从未显示在任务栏中的窗口意外钉入任务栏。
        /// </para>
        /// </summary>
        /// <param name="hwnd">目标窗口句柄。</param>
        /// <param name="originalShownOnTaskbar">窗口加入管理前是否原本显示在任务栏中。</param>
        private static void RestoreHiddenState(IntPtr hwnd, bool originalShownOnTaskbar)
        {
            if (originalShownOnTaskbar)
            {
                WindowHelper.SetExStyleFlag(hwnd, WindowStyles.WS_EX_TOOLWINDOW, false);
                WindowHelper.SetExStyleFlag(hwnd, WindowStyles.WS_EX_APPWINDOW, true);
            }
            else
            {
                // 仅撤销引擎加的 WS_EX_TOOLWINDOW，不补 WS_EX_APPWINDOW
                WindowHelper.SetExStyleFlag(hwnd, WindowStyles.WS_EX_TOOLWINDOW, false);
            }
        }
